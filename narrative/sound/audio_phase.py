...
from contracts.schemas.handoffs import Phase2AudioHandoff
from datetime import datetime
from pathlib import Path
from pydantic import ValidationError
from toolkit.audio.audio_mix import concat_audio_files, mix_audio_layers
from toolkit.audio.bedtrack import compose_bed_track
...
def execute_phase_two(handoff: str | Path | Dict[str, Any] | Phase2AudioHandoff, destination_dir: str | Path | None=None, include_bgm: bool=True) -> Dict[str, Any]:
    ...
    for segment in raw_segments:
        ...
        segment_record = {'segment_id': segment['segment_id'], 'scene_id': segment['scene_id'], 'character_id': segment['character_id'], 'line_id': segment['line_id'], 'text': segment['text'], 'emotion': segment['emotion'], 'audio_file': tts_output['audio_file'], 'start_ms': start_tick, 'end_ms': end_tick, 'duration_ms': span_ms, 'provider': tts_output['provider']}
    ...
    for scene_key in collect_scene_keys(raw_segments):
        scene_segments = [item for item in speech_entries if item['scene_id'] == scene_key]
