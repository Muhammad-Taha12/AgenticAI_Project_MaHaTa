...
from contracts.schemas.characters import CharacterRoster
...
def cast_node(snapshot: Phase1State) -> Dict[str, Any]:
    ...
    scene_keys = [s['scene_id'] for s in story.get('scenes', [])]
    ...
    try:
        ...
        roster: CharacterRoster = structured_llm.invoke(messages + [HumanMessage(content="Now produce the final CharacterRoster JSON. Ensure character_ids are 'char_001', 'char_002', etc. Every scene_id in scenes_appearing_in must match a scene_id from the story exactly. Include global_art_style.")])
