...
from contracts.schemas.stories import StoryOutput
from langchain_core.messages import AIMessage, HumanMessage, SystemMessage
from narrative.storycraft.helpers.plot_checks import check_story_arc, estimate_story_duration
from narrative.storycraft.llm_runtime import resolve_llm, cycle_tool_execution
...
def plot_node(snapshot: Phase1State) -> Dict[str, Any]:
    ...
    try:
        ...
        story: StoryOutput = structured_llm.invoke(messages + [HumanMessage(content="Now produce the final structured StoryOutput JSON. Use scene_ids like 'scene_001', 'scene_002', etc. Every scene must have an arc_position, tone, setting, summary, and estimated_duration_seconds.")])
        logger.info("Story agent done: '%s' — %d scenes, ~%ds", story.title, len(story.scenes), story.total_estimated_duration_seconds)
