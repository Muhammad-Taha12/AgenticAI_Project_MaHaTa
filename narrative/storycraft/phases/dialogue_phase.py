...
from contracts.schemas.scripts import ScriptOutput
...
from narrative.storycraft.helpers.plot_checks import estimate_story_duration
...
def dialogue_node(snapshot: Phase1State) -> Dict[str, Any]:
    ...
    try:
        ...
        script: ScriptOutput = structured_llm.invoke(messages + [HumanMessage(content="Now produce the final ScriptOutput JSON. Include one SceneScript for EVERY scene in the story. scene_ids must match the story exactly. character_ids in dialogue must match the roster exactly. line_ids use format 'line_001', 'line_002', … (reset per scene).")])
